...
public class OneBarElements {
    ...
    private void barText(){
        ...
        if(config.textSettings.showText) {
            ...
            value += Calculations.emojiOrText("text.onebar.health.extra.emoji","text.onebar.health.extra", true, config.textSettings.rawHealth ? (Math.round(playerProperties.healthRaw * 100.0) / 100.0) : Calculations.makeFraction(playerProperties.health, false));
        ...
        if (playerProperties.hasAbsorption)
            value += plus + Calculations.emojiOrText("text.onebar.absorption.extra.emoji","text.onebar.absorption.extra", true, Calculations.makeFraction(playerProperties.absorption, false));
        ...
        if(config.textSettings.showText) { // Separated if because order matters
            if (playerProperties.hasResistance && config.goodThings.showResistance)
                value += plus + Calculations.emojiOrText("text.onebar.resistance.emoji","text.onebar.resistance", false, playerProperties.resistancePercent);
            ...
            if(playerProperties.isWardenNear && config.badThings.showWarden)
                value += minus + Calculations.emojiOrText("text.onebar.warden.emoji","text.onebar.warden",false, Calculations.makeFraction(playerProperties.wardenDanger, false));
            ...
            if (playerProperties.normalFallHurts && config.badThings.showFallHeight && !playerProperties.hasLevitation)
                value += minus + Calculations.emojiOrText("text.onebar.falling.emoji", "text.onebar.falling", false, playerProperties.normalFallHeightDisplay);
            if (playerProperties.hasGlowing && config.badThings.showGlowing)
                value += minus + Calculations.emojiOrText("text.onebar.glowing.emoji","text.onebar.glowing", false, (Object) null);
            if (playerProperties.hasInfested && config.badThings.showInfested)
                value += minus + Calculations.emojiOrText("text.onebar.infested.emoji","text.onebar.infested", false, (Object) null);
            if (playerProperties.hasWeaving && config.badThings.showPostDeathEffects)
                value += minus + Calculations.emojiOrText("text.onebar.weaving.emoji","text.onebar.weaving", false, (Object) null);
            if (playerProperties.hasOozing && config.badThings.showPostDeathEffects)
                value += minus + Calculations.emojiOrText("text.onebar.oozing.emoji","text.onebar.oozing", false, (Object) null);
            if (playerProperties.hasWindCharged && config.badThings.showPostDeathEffects)
                value += minus + Calculations.emojiOrText("text.onebar.windCharged.emoji","text.onebar.windCharged", false, (Object) null);
            if (playerProperties.hasBadOmen && config.badThings.showOmens)
                value += minus + Calculations.emojiOrText("text.onebar.badOmen.emoji","text.onebar.badOmen", false, playerProperties.badOmenLevel);
            if (playerProperties.hasRaidOmen && config.badThings.showOmens)
                value += minus + Calculations.emojiOrText("text.onebar.raidOmen.emoji","text.onebar.raidOmen", false, playerProperties.raidOmenWaves);
            if (playerProperties.hasTrialOmen && config.badThings.showOmens)
                value += minus + Calculations.emojiOrText("text.onebar.trialOmen.emoji","text.onebar.trialOmen", false, playerProperties.trialOmenMinutes);
            if(playerProperties.isVisibleOnLocatorBar && config.otherBars.showLocatability)
                value += minus + Calculations.emojiOrText("text.onebar.locator.emoji","text.onebar.locator", false, (Object) null);
            if (clientProperties.isHardcore)
                value += minus + Calculations.emojiOrText("text.onebar.hardcore.emoji","text.onebar.hardcore", false, (Object) null);
            ...
            if (showHungerParentheses)
                value += parStart;
            if (hasHunger || (playerProperties.hasHungerEffect && config.healthEstimates && !config.disableHunger))
                value += Calculations.emojiOrText("text.onebar.hunger.extra.emoji","text.onebar.hunger.extra", true, Calculations.makeFraction(playerProperties.hunger, false));
        ...
        drawContext.text(textRenderer, value, textX, textY, config.textSettings.textColor, false);
    }
    ...
    private void xpBar(){
        ...
        if(!config.otherBars.adaptiveXpBar || playerProperties.xpLevel > 0){
            ...
            if(playerProperties.xpLevel >= 0 && playerProperties.xpLevel < sizeLimit){
                drawContext.centeredText(textRenderer, String.valueOf(playerProperties.xpLevel), textX, textY, config.otherBars.xpColor);
            }
            else if(playerProperties.xpLevel >= sizeLimit){
                if(client.options.mainHand().get() == HumanoidArm.RIGHT)
                    drawContext.text(textRenderer, String.valueOf(playerProperties.xpLevel), textX - edgeAlignedConst, textY, config.otherBars.xpColor);
                else if(client.options.mainHand().get()  == HumanoidArm.LEFT)
                    drawContext.text(textRenderer, String.valueOf(playerProperties.xpLevel), textX + edgeAlignedConst - client.font.width(String.valueOf(playerProperties.xpLevel)), textY, config.otherBars.xpColor);
            }
            ...
            if(config.otherBars.lapisCounter){
                drawContext.text(textRenderer, lapisText, lapisTextX, lapisTextY, config.otherBars.lapisColor);
            }

            if(config.otherBars.mendingIndicator && playerProperties.isMendingAnything){
                drawContext.centeredText(textRenderer, Calculations.emojiOrText("text.onebar.mending.emoji", "text.onebar.mending", false), textX, mendingTextY, config.otherBars.mendingColor);
            }
        }
    ...
    public void jumpBar(){
        ...
        if(config.textSettings.showText && config.entity.showMountJumpText)
            drawContext.text(textRenderer, roundedHeightInBlocks, textX, textY, config.textSettings.textColor, false);
    }
    ...
    public void dashBar(){
        ...
        if(relativeEndWCooldown > relativeEndW){
            ...
            if(config.textSettings.showText && config.entity.showMountJumpText){
                ...
                drawContext.text(textRenderer, roundedDistanceInBlocks, textX, textY, config.textSettings.textColor, false);
            }
        }
    }
    ...
    private void dashCooldownBar(int relativeEndW, int cooldownTimer){
        if(config.entity.showMountCooldown){
            ...
            if(config.textSettings.showText && config.entity.showMountCooldownText){
                ...
                int textY = clientProperties.dashEndH - 9;
                drawContext.text(textRenderer, text, textX, textY, config.textSettings.textColor, false);
            }
        }
    }
    ...
    public void mountBar(LivingEntity mountEntity){
        ...
        String value = Calculations.emojiOrText("text.onebar.mountHealth.extra.emoji","text.onebar.mountHealth.extra", true, config.textSettings.rawHealth ? (Math.round(mountRawHealth * 100.0) / 100.0) : Calculations.makeFraction(health, false));
        int textX = clientProperties.baseEndW - client.font.width(value);
        ...
        if(config.textSettings.showText) drawContext.text(textRenderer, value, textX, textY, config.textSettings.textColor, false);
    ...
    private void debugText(String value){
        drawContext.text(textRenderer, value, clientProperties.baseEndW + 15, clientProperties.baseStartH + 1, config.textSettings.textColor);
    }
}
